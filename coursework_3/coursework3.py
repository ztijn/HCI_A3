...
class Dialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.transient(parent)  # Bind parent window
        self.title('Change Credentials')
        self.parent = parent

        self.body = tk.Frame(self)  # body for buttons and input
        self.rows = 0
        while self.rows < 7:
    ...
    def apply(self):
        ...
        if e1 and e2 and e3 and e4:
            ...
            messagebox.showerror("Error:", "All fields need to be filled in")
...
class MyStreamListener(tweepy.StreamListener):
    ...
    def on_error(self, code_status):
        if code_status == 420:  # if request limit reached disconnect stream
            return False
        if code_status == 179:
            pass
...
class MainWindow(ttk.Notebook):
    def __init__(self, master):
        super().__init__(master)
        master.title('HCI Coursework 3: Twitter Conversation Analysis')
        self.part1 = Part1(master)
        self.part2 = Part2(master)
        self.add(self.part1, text='Conversations')
...
class Part1(tk.Frame):
    ...
    def get_conversation(self, tweet, convo, participants):
        ...
        try:
            # Check if there is reply to tweet
    ...
    def get_location(self, address):
        ...
        if geo_address is not None:
            point_address = geopy.Point(geo_address.latitude, geo_address.longitude)
            if self.address_radius.get() and self.address_radius.get() is not None and re.match(
                ...
                n = distance.destination(point=point_address, bearing=0)
                e = distance.destination(point=point_address, bearing=90)
                s = distance.destination(point=point_address, bearing=180)
                w = distance.destination(point=point_address, bearing=270)
                location = [n.latitude, w.longitude, s.latitude, e.longitude]
            else:
                location = [geo_address.latitude, geo_address.longitude, geo_address.latitude, geo_address.longitude]
            return location
    ...
    def get_filter(self):
        ...
        if search_term is not None:
            if len(search_term) == 0:
                ...
                tweet_filter['track'] = search_term.split(', ')
                filters.append(' '.join(search_term.split(', ')))
